...
DESCRIPTION
	This programme fetches updated rates for stocks, physical and digital 
	currencies from	<alphavantage.co> APIs.

	It can also return daily/weekly and monthly time series (date, open, 
	high, low, close and volume) of the stock or currency specified, cover-
	ing 20+ years of historical data. The most recent data point is the
...
LIMITS
	API rate limits are 5 API requests per minute or 500 API requests per 
	day for free accounts, according to <https://www.alphavantage.co/premium/>.
...
OPTIONS
	...
	-jj 	Debug, print raw JSON; twice to try and get CSV data instead.
