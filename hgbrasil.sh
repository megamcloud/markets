...
SINOPSE
	...
	A opção padrão é uma visão geral do mercado e não requer uma chave de API
	e nenhum argumento para o script.

	Para puxar preços de ações específicas e das taxas de CDI e Selic, é ne-
	cessária a criação de uma chave de API grátis em <console.hgbrasil.com/keys>.
	Defina a variável de ambiente 'HGBAPIKEY' ou a respectiva variável na 
	cabeça do código fonte deste script.
...
EXONERAÇÃO DE RESPONSABILIDADE
	Extraído do website <console.hgbrasil.com/documentation/finance>:
		...
		todos os dados antes de efetuar qualquer ação que possa ser afe-
		tada por estes valores, assim como demais endpoints da API.\"
